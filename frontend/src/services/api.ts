...
export const getLeaderboard = async (limit: number=50, cursor: string="")=>{
    const response = await axios.get<{resp: Leaderboard}>(`${apiURLLeaderboard}?cursor=${cursor}&limit=${limit}`)
    return response.data.resp
}

export const searchUsers = async (limit: number=50, cursor: string="", query: string="")=>{
    const response = await axios.get<{resp: Leaderboard}>(`${apiURLSearch}?cursor=${cursor}&limit=${limit}&q=${query}`)
    return response.data.resp
}